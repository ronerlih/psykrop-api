...
module.exports = {
  ...
  analyseImage: async function(image, id, saveImageLocaly) {
    return new Promise(async (resolve, reject) => {
      ...
      let zerosMat = new cv.Mat(
        src.rows,
        src.cols,
        cv.CV_8UC1,
        new cv.Scalar(0)
      );
      let onesMat = new cv.Mat(
        src.rows,
        src.cols,
        cv.CV_8UC1,
        new cv.Scalar(255)
      ...
      arrayofMats.forEach((channel, channelIndex) => {
        ...
        if (channelIndex != 3) {
          ...
          if (saveImageLocaly) {
            ...
            cv.circle(
              ...
              4,
              new cv.Scalar(
                resultsOptions[channelIndex].color[0],
                resultsOptions[channelIndex].color[1],
                resultsOptions[channelIndex].color[2],
                255
              ),
              5,
              cv.LINE_8,
        ...
        [
          resultObject[resultsOptions[channelIndex].channelName]
            .distanceToCenter,
          resultObject[resultsOptions[channelIndex].channelName].balancePercent
        ] = calcBalancePercentage(src, COB);
        ...
        if (saveImageLocaly) {
          ...
          resultObject[resultsOptions[channelIndex].channelName].url =
            ("0" + id).slice(-2) +
            "-" +
            resultsOptions[channelIndex].channelName +
            ".jpg";
        }
        channel.delete();
        vecToMerge.delete();
      ...
      if (saveImageLocaly) {
        ...
        resultObject.imageFeedback =
          ("0" + id).slice(-2) + "-image-feedback.jpg";
      }

      // average balance
      const aveCenter = weightedAverageThree(...channelsCenters);
      [
        resultObject.distanceToCenter,
        resultObject.balanceAllCoefficients
      ...
      function weightedAverageThree(_redPoint, _greenPoint, _bluePoint) {
        return new cv.Point(
          (_redPoint.x * RED_COEFFICIENT +
            _greenPoint.x * GREEN_COEFFICIENT +
            _bluePoint.x * BLUE_COEFFICIENT) /
            (RED_COEFFICIENT + GREEN_COEFFICIENT + BLUE_COEFFICIENT),
          (_redPoint.y * RED_COEFFICIENT +
            _greenPoint.y * GREEN_COEFFICIENT +
            _bluePoint.y * BLUE_COEFFICIENT) /
            (RED_COEFFICIENT + GREEN_COEFFICIENT + BLUE_COEFFICIENT)
        );
      }
    });
    function calcBalancePercentage(mat, point) {
      let totalDistance = Math.sqrt(
        (mat.cols / 2) * (mat.cols / 2) + (mat.rows / 2) * (mat.rows / 2)
      );

      let diff = Math.sqrt(
        (point.x - mat.cols / 2) * (point.x - mat.cols / 2) +
          (point.y - mat.rows / 2) * (point.y - mat.rows / 2)
      );
